...
SMALL_GRAMMAR = {
    ...
    '<array_expr>' : ['<identifier>[<arithExpr>]'],
    '<if>' : ['if(<bool_arith>) <stmt> else <stmt>'],
    '<while>' : ['while(<boolExpr>) <sequence>'],
    '<sequence>' : ['{<stmtlist>}'],
    '<returnStmt>' : ['return <expr>;'],
    
    '<expr>' : ['<bool_arith>','<array_expr>'],
    '<bool_arith>' : ['<arithExpr>', '<boolExpr>'],
    '<arithExpr>' : ['<arithExpr> <op> <arithExpr>', '<noprnd>'],
    '<boolExpr>' : ['<boprnd> <rop> <boprnd>', '<arithExpr> <rop> <arithExpr>', '<boprnd>'],
